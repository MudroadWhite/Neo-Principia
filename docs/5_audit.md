...
- **\*14.32.** It has been assumed that \*14.32 is undergoing proof with same style as \*14.31, besides the fact that \*14.32 is bidirectional(using `↔`). The reverse direction involves using \*14.21(as the only way provided) and \*14.1. \*14.21 is taking a whole single `ιφ` as its premise; the hypothesis of our goal however, in our representation, is `([ ι φ | ιφ => ¬ χ ιφ]) ↔ ¬ ([ ι φ | ιφ => χ ιφ])` which involves two `ι`. Therefore we need to have a way to transform the two `ι`s into a single `ι`. What will be required beneath the re-scoping is a theorem of `(∃ x, P x) ∧ (∃ x, Q x) -> (∃ x, P x ∧ Q x)`, which is even intuitively not always true. We conclude the reverse direction of this theorem unprovable.
